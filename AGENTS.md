...
## Testing Guidelines
There is no formal test suite; rely on manual verification. Open `index.html` via a static file server (`python3 -m http.server 8080`) and confirm animations, review ticker population, and responsive breakpoints. When touching the worker, mock `fetch` responses or hit the service with `curl -H "Origin: http://localhost:8080" http://localhost:8787/api/reviews` to verify CORS behavior. Document any manual scenarios you executed in the pull request description.
